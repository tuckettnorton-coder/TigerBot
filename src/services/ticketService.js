...
function allTicketStaffRoles(guild) {
  const names = Object.values(TICKET_TYPES).flatMap((ticket) => ticket.pingRoles || []);
  return resolveRoles(guild, names);
}
...
export function getTicketFromChannel(channel) {
  ...
  try {
    return JSON.parse(channel.topic.slice(OPEN_MARKER.length));
  } catch {
    return null;
  }
}
  ...
  for (const name of names) {
    ...
    if (!category) {
      category = await guild.channels.create({
        ...
        reason: 'TigerBot automatic ticket category setup',
      });
    }
...
async function ensureUtilityChannel(guild, name) {
  ...
  const staffRoles = allTicketStaffRoles(guild);
  const overwrites = [
    { id: guild.roles.everyone.id, deny: [PermissionFlagsBits.ViewChannel] },
    ...(guild.ownerId ? [{ id: guild.ownerId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory] }] : []),
    ...staffRoles.map((role) => ({
      id: role.id,
  ...
  channel = await guild.channels.create({
    ...
    reason: 'TigerBot automatic ticket utility channel setup',
  });
  return channel;
}
...
export async function findExistingTicket(guild, userId, categoryName) {
  try {
    ...
    // Cached channels are still usable if fetching the collection fails.
  }

  return guild.channels.cache.find((channel) => {
    const ticket = getTicketFromChannel(channel);
    return channel.type === ChannelType.GuildText
      && ticket?.openerId === userId
      && ticket?.categoryName?.toLowerCase() === categoryName.toLowerCase();
  }) || null;
}
...
export async function createTicketChannel({ guild, user, typeId, answers = {} }) {
  ...
  if (Object.keys(answers).length && ticket.form?.length) {
    const answerFields = ticket.form.map((field) => ({
      name: field.label,
...
export async function requestClose(channel, member) {
  ...
  const alreadyPending = recent?.some((message) => message.embeds?.some((embed) => embed.title === 'Close Request'));
  if (alreadyPending) return null;

  const embed = new EmbedBuilder()
    .setTitle('Close Request')
    .setDescription(
      `Staff member ${member} has requested to close this ticket.\n\n<@${ticket.openerId}>\n\n**Confirmation**\n> Would you like to close this ticket?`,
    );
...
export async function closeTicket(channel, actor) {
  ...
  await transcriptChannel.send({
    content: `📜 Transcript for **${channel.name}** • closed by ${actor}`,
    files: [transcript],
  });
...
export async function logTicket(guild, message) {
  try {
    ...
    // Logging must never prevent a ticket from being created or closed.
  }
}
