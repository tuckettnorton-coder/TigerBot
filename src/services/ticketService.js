...
function categoryByName(guild, name) {
  return guild.channels.cache.find(c => c.type === ChannelType.GuildCategory && c.name.toLowerCase() === name.toLowerCase());
}
...
function ticketName(user) {
  ...
  return `ticket-${clean}-${Math.floor(1000 + Math.random() * 9000)}`;
}
...
export async function createTicketChannel({ guild, user, typeId, answers = {} }) {
  ...
  const metadata = { typeId, openerId: user.id, categoryName: ticket.categoryName, claimedBy: null, createdAt: new Date().toISOString() };
  const channel = await guild.channels.create({
    name: ticketName(user),
    type: ChannelType.GuildText,
    parent: category.id,
    topic: `${OPEN_MARKER}${JSON.stringify(metadata)}`,
    permissionOverwrites: overwrites,
    reason: `Ticket opened by ${user.tag} (${ticket.label})`,
  });

  const mentions = roles.map(r => `<@&${r.id}>`).join(' ');
  const welcome = new EmbedBuilder()
    .setTitle(ticket.label)
    .setDescription(ticket.welcomeMessage.replace('{user}', user.toString()).replace(/@(Helper|Buyer\/Seller|Owner|Trial \| Helper|Admin|Head \| Manager|Partner \| Manager|Trial \| PM|Builder\/Digger|Trial \| Builder\/Digger|Mod)/g, ''))
    .addFields({ name: 'Staff', value: mentions || 'Staff' })
...
export async function requestClose(channel, member) {
  ...
  if (!isStaffForTicket(member, TICKET_TYPES[ticket.typeId])) throw new Error('Only the ticket staff team can request a close.');

  const embed = new EmbedBuilder()
    .setTitle('Close Request')
    .setDescription(`Staff member ${member} has requested to close this ticket\n${member.guild.members.cache.get(ticket.openerId) || `<@${ticket.openerId}>`}`)
...
export async function closeTicket(channel, actor, transcriptChannelId) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${channel.name}</title><style>body{font-family:Arial;background:#111;color:#eee;padding:24px}article{padding:12px;border-bottom:1px solid #333}small{color:#aaa}pre{white-space:pre-wrap;font:inherit}</style></head><body><h1>${channel.name}</h1><p>Closed by ${actor.tag} • ${new Date().toISOString()}</p>${messages.map(m => `<article><b>${escapeHtml(m.author.tag)}</b> <small>${m.createdAt.toISOString()}</small><pre>${escapeHtml(m.content || '[embed/attachment]')}</pre></article>`).join('')}</body></html>`;
