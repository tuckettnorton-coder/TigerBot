...
function normalize(expression) {
  return String(expression).replace(/,/g, '').replace(/×/g, '*').replace(/÷/g, '/').replace(/([0-9]+(?:\.[0-9]+)?)([KMBT])\b/gi, (_, number, suffix) => `${Number(number) * SUFFIXES[suffix.toUpperCase()]}`);
}

function calculate(expression) {
  const normalized = normalize(expression);
  if (!normalized || !/^[0-9+\-*/%^().\s]+$/.test(normalized)) throw new Error('Enter a valid calculation.');
...
function rows() {
  return [
    ...
    new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('ticket_calc_backspace').setLabel('⌫').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('ticket_calc_enter').setLabel('Enter').setStyle(ButtonStyle.Success)),
  ];
}
...
function embed(expression, result = null) {
  return new EmbedBuilder().setTitle('🧮 Donut SMP Calculator').setDescription(`**Type with your keyboard or use the buttons below.**\n\n**Calculation**\n\`${expression || '—'}\`\n\n**Result**\n### ${result === null ? '—' : formatResult(result)}`).setFooter({ text: 'TigerBot • Private calculator • Support only' });
}
...
async function execute(interaction) {
  ...
  const additions = {
    ticket_calc_0:'0',ticket_calc_1:'1',ticket_calc_2:'2',ticket_calc_3:'3',ticket_calc_4:'4',ticket_calc_5:'5',ticket_calc_6:'6',ticket_calc_7:'7',ticket_calc_8:'8',ticket_calc_9:'9',ticket_calc_decimal:'.',ticket_calc_plus:' + ',ticket_calc_minus:' - ',ticket_calc_multiply:' × ',ticket_calc_divide:' ÷ ',ticket_calc_k:'K',ticket_calc_m:'M',ticket_calc_b:'B',ticket_calc_t:'T'
  };
...
const ids = [...Array.from({ length: 10 }, (_, i) => `ticket_calc_${i}`), 'ticket_calc_decimal','ticket_calc_plus','ticket_calc_minus','ticket_calc_multiply','ticket_calc_divide','ticket_calc_k','ticket_calc_m','ticket_calc_b','ticket_calc_t','ticket_calc_clear','ticket_calc_backspace','ticket_calc_enter'];
