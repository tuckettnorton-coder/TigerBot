...
import { registerTicketEphemeral, clearTicketEphemeralLater } from '../../utils/ticketEphemeral.js';
...
export default { name:'ticket_select', async execute(interaction) {
  try {
    ...
    if(typeId==='advertisement'){setPaidAdDraft(interaction.user.id,{});await interaction.reply({content:'### 💰 Paid Advertisement\n**What paid advertisement would you like?**\n\nChoose one of the current plans below.',components:[buildPaidAdPlanMenu()],ephemeral:true});registerTicketEphemeral(interaction.user.id,interaction);return;}
    if(typeId==='buying_selling_spawners'){await interaction.reply({content:'### 💸 Buying/Selling Spawners\nFirst, select whether you want to **Buy** or **Sell**.',components:[buildSpawnerTradeMenu()],ephemeral:true});registerTicketEphemeral(interaction.user.id,interaction);return;}
    if(typeId==='digging_services'){await interaction.reply({content:'### ⛏️ Digging Service\n**Do you have an area?**\nSelect an option below.',components:[buildYesNoMenu('digging_area','Do you have an area?','I have an area/location','I need the builder to choose the area')],ephemeral:true});registerTicketEphemeral(interaction.user.id,interaction);return;}
