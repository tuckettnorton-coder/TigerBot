...
import { getPaidAdDraft } from '../../utils/paidAdDrafts.js';
...
function addonMenu() {
  ...
  if (getPaidAdDraft(globalThis.__paidAdCurrentUserId || '')?.plan !== 'premium') {
    options.unshift(new StringSelectMenuOptionBuilder().setLabel(`🕒 Scheduled Posting — $${Number(p.scheduled).toFixed(2)}`).setDescription('Choose a specific posting time').setValue('scheduled'));
  }
  return new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('paid_ad_addon_choice').setPlaceholder('Choose another add-on...').addOptions(options));
}
