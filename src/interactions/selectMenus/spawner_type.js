...
export default {
  ...
  async execute(interaction, client, args) {
    ...
    if (!['buy', 'sell'].includes(trade) || !spawnerLabel) return interaction.reply({ content: '❌ Invalid spawner selection.', ephemeral: true });
    const modal = new ModalBuilder()
      ...
      .addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('amount').setLabel('Amount of Spawners (Minimum 3)').setStyle(TextInputStyle.Short).setPlaceholder('Example: 3, 64, 128, 1,000, 2.5k').setRequired(true)),
