...
export default {
  ...
  async execute(interaction, client, args) {
    ...
    if (!['buy', 'sell'].includes(trade) || !LABELS[spawnerType] || !amount) {
      return interaction.reply({ content: '❌ Invalid amount selection.', ephemeral: true });
    }

    if (amount === 'custom') {
      return interaction.showModal(buildIgnModal(trade, spawnerType));
    }

    // This branch is used after the first spawner has been entered.
    if (args?.[0] === 'more') {
        ...
        components: [
          new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
              .setCustomId(`spawner_type:${actualTrade}`)
              .setPlaceholder('Choose a spawner type...')
              .addOptions(
