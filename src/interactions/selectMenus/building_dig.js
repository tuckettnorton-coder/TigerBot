...
export default { name:'building_dig', async execute(interaction,client,args) {
  ...
  const modal=new ModalBuilder().setCustomId(`ticket_form:building_services:digstart:${schematic}`).setTitle('Digging Area').addComponents(
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('area_size').setLabel('Area size (width x length x height)').setStyle(TextInputStyle.Short).setPlaceholder('100 × 10 × 50 or 100 10 50').setRequired(true)),
