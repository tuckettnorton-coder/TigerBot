...
function parseMiddlemanSpawnerTrade(value){
  const raw=String(value??'').trim().toLowerCase();
  const spawnerType=raw.includes('iron golem')||raw.includes('irongolem')||raw.includes('iron')?'irongolem':raw.includes('creeper')?'creeper':raw.includes('skeleton')?'skeleton':null;
  const trade=raw.includes('sell')?'sell':raw.includes('buy')?'buy':null;
  return {spawnerType,trade};
}
...
export default { name:'ticket_form', async execute(interaction,client,args){try{
 ...
 if(typeId==='building_services'){
   ...
   if(step==='final'){
     ...
     const answers={what_built:draft.whatBuilt,is_farm:draft.isFarm?'Yes':'No',price_value:draft.isFarm?draft.priceInput:'—',schematic:schematic==='yes'?'Yes':'No',area_dug:area==='yes'?'Yes':'No',digging_requested:dig==='yes'?'Yes':'No',digging_area_size:draft.areaSize||'—',good_coords:dig==='yes'?(good==='yes'?'Yes':'No'):'—',region:dig==='yes'?REGION_LABELS[region]:'—',ign};
 ...
 if(typeId==='middleman'){
   ...
   const spawnersInvolved=interaction.fields.getTextInputValue('spawners_involved').trim().toLowerCase();
   const amountInput=interaction.fields.getTextInputValue('spawner_amount').trim();
   const tradeInput=interaction.fields.getTextInputValue('spawner_trade').trim();
   if(!yourIgn||!personIgn)return interaction.reply({content:'❌ Please enter both IGNs.',ephemeral:true});
   if(!['yes','no'].includes(spawnersInvolved))return interaction.reply({content:'❌ **Are spawners involved?** must be **Yes** or **No**.',ephemeral:true});

   let calculation=null;
   let answers={your_ign:yourIgn,person_ign:personIgn,spawners_involved:spawnersInvolved==='yes'?'Yes':'No',spawner_amount:amountInput,spawner_trade:tradeInput};

   if(spawnersInvolved==='yes'){
     const amount=parseSpawnerAmount(amountInput);
     if(amount===null||!Number.isInteger(amount)||amount<1)return interaction.reply({content:'❌ Please enter a valid whole-number spawner amount, such as **3**, **64**, or **128**.',ephemeral:true});
     const {spawnerType,trade}=parseMiddlemanSpawnerTrade(tradeInput);
     if(!spawnerType||!trade)return interaction.reply({content:'❌ For the last field, enter the **spawner type and Buy/Sell**, for example **Skeleton Sell** or **Creeper Buy**.',ephemeral:true});
     calculation=calculateMiddlemanSpawner({trade,spawnerType,amount});
   } else {
     const value=parseAmount(tradeInput);
     if(value===null||!Number.isFinite(value)||value<0)return interaction.reply({content:'❌ Since there are no spawners, enter the **total value involved** in the last field, such as **100M**, **1.5B**, or **2500000**.',ephemeral:true});
   ...
   if(spawnersInvolved==='yes'){
     await addCalculationToPanel(result.channel,ticket.label,'🤝 Automatic Middleman Fee Calculation',buildMiddlemanSpawnerCalculationMessage(calculation));
   } else {
     await addCalculationToPanel(result.channel,ticket.label,'🤝 Automatic Middleman Fee Calculation',buildMiddlemanOtherCalculationMessage(calculation));
   }
   ...
   await interaction.editReply(`✅ Ticket created: ${result.channel}\n💰 **Middleman fee: ${spawnersInvolved==='yes'?calculation.middlemanFeeFormatted:calculation.middlemanFeeFormatted}**`);
