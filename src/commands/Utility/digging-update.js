...
const DEFAULT_PRICES = { perBlock: '$1000', goodCoords: '20M', customRegion: '10M' };
const DEFAULT_NOTES = [
...
function cloneDefaults() {
  return { prices: { ...DEFAULT_PRICES }, notes: DEFAULT_NOTES, refundPolicy: DEFAULT_REFUND, ticket: DEFAULT_TICKET };
}
...
export function loadDiggingPrices() {
  try {
    ...
    return { prices: { ...defaults.prices, ...(parsed.prices || {}) }, notes: parsed.notes || defaults.notes, refundPolicy: parsed.refundPolicy || defaults.refundPolicy, ticket: parsed.ticket || defaults.ticket };
  } catch { return cloneDefaults(); }
}

function saveDiggingPrices(data) { fs.writeFileSync(DATA_FILE, `${JSON.stringify(data, null, 2)}\n`, 'utf8'); }

function loadMessageId() {
  try { return JSON.parse(fs.readFileSync(MESSAGE_FILE, 'utf8')).messageId || null; } catch { return null; }
}
...
export function formatDiggingPriceMessage(data) {
  ...
  return ['# Digging Prices', `- ${prices.perBlock} Per block`, `- ${prices.goodCoords} For good coords`, `- ${prices.customRegion} For custom region`, '', '# Additional Notes', notes, '', '# 💰 Refund Policy', refundPolicy, '', `### ${ticket}`].join('\n');
}
...
export async function execute(interaction, guildConfig, client) {
  ...
  const saved = await getLatestUpdateData(client, interaction.guildId, 'diggingPrices', null);
  await interaction.showModal(buildDiggingPriceModal(saved || loadDiggingPrices()));
}

export default { data, execute };

export function persistDiggingPrices(data) { saveDiggingPrices(data); }
...
export async function postDiggingPrices(client, data) {
  ...
  if (!channel || typeof channel.send !== 'function') throw new Error(`Digging price channel ${DIGGING_PRICE_CHANNEL_ID} is not a sendable channel.`);
